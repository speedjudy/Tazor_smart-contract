...
contract TazorTreasury is Ownable {
    ...
    constructor (
        ...
        // isLiquidityToken[ _TAZORDAI ] = true;
        // liquidityTokens.push( _TAZORDAI );
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        if( swapAndLiquifyEnabled == true){
            if (createLPtoken[_token]){
                ...
                addLiquidityTokens(_token, TAZOR, quarterAmount, newTazorBalance);
                ...
                addLiquidityTokens(_token, TAZ, quarterAmount, newTazBalance);
            }
        }
    }
    ...
    function swapTokensForEth(uint256 tokenAmount, address token0) private {
    ...
    function addLiquidityETH(address token, uint256 tokenAmount, uint256 ethAmount) private {
    ...
    function withdraw( uint _amount, address _token ) external {
        ...
        ITAZORERC20( TAZOR ).burnFrom( msg.sender, value );
    ...
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            ...
            value_ = _amount.mul( 10 ** IERC20( TAZOR ).decimals() ).div( 10 ** IERC20( _token ).decimals() );

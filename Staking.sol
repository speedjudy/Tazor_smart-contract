...
contract Ownable is IOwnable {
    ...
    mapping(address => bool) internal _owner_;
    ...
    function transferOwnership( address newOwner_ ) public virtual override onlyPolicy() {
        require( newOwner_ != address(0), "Ownable: new owner is the zero address");
        setOwnership(newOwner_);
    }

    function setOwnership( address newOwner ) internal {
        _owner = newOwner;
        emit TransferOwnership( _owner, newOwner );
        _owner_[newOwner] = true;
    }
}
...
contract TazorDAOStaking is TazorAccessControlled, Ownable, ReentrancyGuard {
     ...
     */
    function tazStakedWithdraw(uint256 _amount) external onlyPolicy {
     ...
     * @dev Only callable by owner. Needs to be for emergency.
     */
    function tazorStakedWithdraw(uint256 _amount) external onlyPolicy {
